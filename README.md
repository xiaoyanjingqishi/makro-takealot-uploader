...
```
- **本机控制台地址**：`http://localhost:8001` 或 `http://localhost`
- **局域网免端口访问**：`http://<宿主机IP>/`（例如 `http://192.168.110.145/`，同局域网手机、平板或其他电脑直接打开，无需输入端口号）
- **局域网直连端口**：`http://<宿主机IP>:8001/`
- **API 接口文档**：`http://localhost:8001/api/docs`

---

### 第二步：加载与配置 Chrome 浏览器插件
1. 打开 Chrome 浏览器，访问 `chrome://extensions/` 并开启右上角 **【开发者模式】**。
2. 点击左上角 **【加载已解压的扩展程序】**，选择本项目中的 **`extension`** 目录。
3. 点击浏览器右上角插件图标打开面板：
   - 若在宿主机使用，默认连接 `http://localhost:8001`，显示绿色「服务在线」；
   - 若在局域网内其他电脑使用，点击【局域网反代 (:80)】或【局域网 :8001】预设按钮（或手动输入宿主机 IP 并点击保存），即可无缝跨设备采集与同步。
